...
public class Morphology {
        ...
        for (String word : words) {
            if (word.matches(REG_RUS) && (word.trim().length() > 1 || word.trim().matches("я"))) {
                ...
                for (String w : wordInfo) {
                    List<String> wordBaseForms = !getRusInfo(w) ? lucMorphRus.getNormalForms(word.trim()) : null;
                ...
                for (String w : wordInfo) {
                    List<String> wordBaseForms = !getEngInfo(w) ? lucMorphEng.getNormalForms(word.trim()) : null;
                    getLemmas(wordBaseForms);
                }
            }
        }
    ...
    private HashMap<String, Integer> getLemmas (List<String> wordBaseForms) {
    ...
    private TreeMap<String, String> getLemmasWords( List<String> wordBaseForms, String word) {
