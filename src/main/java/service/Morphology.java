        ...
        for (String word : words) {
            if (word.matches(REG_RUS) && (word.trim().length() > 1 || word.trim().matches("я"))) {
                ...
                for (String w : wordInfo) {
                    List<String> wordBaseForms = !getRusInfo(w) ? lucMorphRus.getNormalForms(word.trim()) : new ArrayList<>();
                ...
                for (String w : wordInfo) {
                    List<String> wordBaseForms = !getEngInfo(w) ? lucMorphEng.getNormalForms(word.trim()) : new ArrayList<>();
                    getLemmas(wordBaseForms);
                }
            }
        }
