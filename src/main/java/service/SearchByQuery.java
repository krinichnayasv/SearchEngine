...
public class SearchByQuery {
    ...
    public List<RelevancePage> getResultOfSearch()
        ...
        if(!values.isEmpty()) {
            ...
            for(Page page : absRelev.keySet()) {
                Callable task = () -> {
                  return  getRelevancePageResult(page,absRelev.get(page));
                };
    ...
    public String getSnippet(Document document) throws IOException {
        ...
        if ((positionOfWord.lastKey() - positionOfWord.firstKey()) <= 20
            ...
            while (snip.length() <= MAX_SIZE_SNIPPET) {
    ...
    public TreeMap<Integer, String> getPositionOfWord(String[] wordsOfPage,  String contentOfPage) throws IOException {
    ...
    public StringBuilder getResultPages(List<String> lemmasForSearch)  throws SQLException {
        ...
        StringBuilder values = new StringBuilder();
            for(String lemma : lemmasForSearch) {
                StringBuilder pages = getEndPagesBySites(n, values, lemma, sitesList);
                values = pages;
                n++;
            }
    ...
    public StringBuilder getEndPagesBySites(int n, StringBuilder values, String lemma, StringBuilder sites)
            throws SQLException {
        StringBuilder newValues;
        if(n == 0) {
            newValues = DBConnection.getPagesBegin(lemma, sites);
            if(newValues == null || newValues.toString().isEmpty()) {
                ...
                ResultSet rsPages = DBConnection.getConnection().createStatement()
                        .executeQuery("SELECT * FROM page WHERE site_id in (" + sites + ")");
                while (rsPages.next()) {
            ...
            newValues = DBConnection.getPages(lemma, values);
            if(newValues == null || newValues.toString().isEmpty()) {
                lemmaNotFound.add(lemma);
                lemmasForSnippet.remove(lemma);
                newValues = values;
