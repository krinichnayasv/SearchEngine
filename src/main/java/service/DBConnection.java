...
public class DBConnection {
    ...
    public static float getRelevanceOfPage(List<String> lemmasForSearch, int id) throws SQLException {
        float absRel = 0;
    ...
    public static Map<String, String> getSiteOfPage(Page page) throws SQLException {
        Map<String, String> siteParam = new HashMap<>();
        ResultSet rs = DBConnection.getConnection().createStatement()
                .executeQuery("SELECT s.* FROM page p, site s WHERE p.id = '" + page.getId() + "' AND " +
                        "s.id = p.site_id");
        while (rs.next()) {
            siteParam.put(rs.getString("url"), rs.getString("name"));
        }
        rs.close();
        return siteParam;
    }
